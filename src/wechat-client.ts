...
import { debugLog } from './wechat-logger.js'
import type { Credentials, IncomingMessage, MessageItem, WeixinMessage } from './wechat-types.js'
import {
  loadContextTokens,
...
export class WeixinClient {
  ...
  async sendTyping(userId: string, status: 1 | 2 = 1): Promise<void> {
    const ticket = await this.getTypingTicket(userId)
    if (!ticket) return
    await apiSendTyping(this.baseUrl, this.token, userId, ticket, status)
  }
}
